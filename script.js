...
function downloadTemplate() {
  ...
  XLSX.writeFile(wb, "InfiTech_Student_Result_Format.xlsx");
}

document
  .getElementById("frontLogoInput")
  .addEventListener("change", function (e) {
    if (e.target.files[0]) {
      const reader = new FileReader();
      reader.onload = function (ev) {
        frontLogoSrc = ev.target.result;
      };
...
document
  .getElementById("backLogoInput")
  .addEventListener("change", function (e) {
    if (e.target.files[0]) {
      const reader = new FileReader();
      reader.onload = function (ev) {
        backLogoSrc = ev.target.result;
      };
      reader.readAsDataURL(e.target.files[0]);
    }
  });
...
document.getElementById("photosInput").addEventListener("change", function (e) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const reader = new FileReader();
    reader.onload = function (ev) {
      studentPhotos[fileName] = ev.target.result;
    };
...
function generateReportCards() {
  if (excelData.length === 0) {
    alert("Data Empty. Please upload Excel.");
    return;
  }
  ...
  excelData.forEach((student) => {
    ...
    let percent =
      max_grand_total > 0
        ? ((sum_grand / max_grand_total) * 100).toFixed(2)
        : 0;
    let resultStatus = percent > 33 ? "Passed" : "Failed";

    let photoSrc =
      studentPhotos[student.RollNo] ||
      "https://via.placeholder.com/150?text=No+Photo";
    let frontImgTag = frontLogoSrc
      ? `<img src="${frontLogoSrc}" style="height:90px;">`
      : "<h2>LOGO</h2>";
    let backImgTag = backLogoSrc
      ? `<img src="${backLogoSrc}" class="b-logo">`
      : `<div class="b-logo">LOGO</div>`;
    let wmTag = backLogoSrc
      ? `<img src="${backLogoSrc}" class="watermark">`
      : "";
    ...
    let reportCardHTML = `
        <div class="page">
            ...
            <div class="front-content">
                <div class="school-header">
                    ${frontImgTag}
                    <h1 class="school-name">SITARA PUBLIC SCHOOL</h1>
                    <div class="school-address">Madina Colony Muzaffarnagar U.P.</div>
                </div>
                <div class="report-title">
                    Progress Report Card <br>
                    <span style="font-size: 20px; font-weight: normal;">Session (${session})</span>
                </div>
                <div style="text-decoration: underline; font-weight: bold; margin-top: 10px;">Student Profile</div>
                <div class="student-photo-frame"><img src="${photoSrc}" alt="Student Photo"></div>
                
                <table class="profile-table">
                    <tr><td>Name</td><td>${student.Name}</td></tr>
                    <tr><td>Father's Name</td><td>${
                      student.FatherName
                    }</td></tr>
                    <tr><td>Mother's Name</td><td>${
                      student.MotherName || "-"
                    }</td></tr>
        ...
        <div class="page">
            ...
            <table class="marks-header-table">
                <tr>
                    <td style="width:50%;">NAME: <span style="font-weight:normal; text-transform:uppercase;">${
                      student.Name
                    }</span></td>
                    <td style="width:50%;">FATHER: <span style="font-weight:normal; text-transform:uppercase;">${
                      student.FatherName
                    }</span></td>
                </tr>
                <tr>
                    <td>CLASS: <span style="font-weight:normal; text-transform:uppercase;">${
                      student.Class
                    }</span></td>
                    <td>ROLL NO: <span style="font-weight:normal;">${
                      student.RollNo
                    }</span></td>
                </tr>
            </table>
            ...
            <div class="footer-info">
                ...
                <div class="footer-grid">
                    <div style="border-right: 1px solid #ccc; padding-right: 15px;">
                        <div class="footer-row"><span>Remarks:</span> <b>${
                          student.Remarks || "Very Good"
                        }</b></div>
                        <div class="footer-row"><span>Promoted to class:</span> <b>${
                          student.PromotedTo || "Next Class"
                        }</b></div>
                    ...
                    <div style="padding-left: 15px;">
                        ...
                        <div class="footer-row"><span>Attendance:</span> <b>${
                          student.Attendance || "-"
                        }</b></div>
                        <div class="footer-row"><span>Conduct:</span> <b>Good</b></div>
                        <div class="footer-row"><span>Rank:</span> <b>${
                          student.Rank || "-"
                        }</b></div>
                    </div>
                </div>
            ...
            <div class="signatures">
                <div class="sign-box"><div class="sign-line">Class Teacher Signature</div></div>
                <div class="sign-box"><div class="sign-line">Checked by</div></div>
                <div class="sign-box"><div class="sign-line">Principal<br>STAMP & SIGNATURE</div></div>
            </div>
